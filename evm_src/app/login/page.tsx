...
export default function LoginPage() {
  ...
  async function studentManualLogin(e: React.FormEvent) {
    e.preventDefault();
    if (manualId.length !== 5 || !manualPassword) {
      showToast("⚠️ Please enter your 5-digit roll number and password.");
      return;
    }

    setIsLoading(true);
    const fullStudentId = `VIT-26-${manualId}`;

    try {
      const res = await fetch("/api/auth/manual-login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ identifier: fullStudentId, password: manualPassword }),
      });
              ...
              <form onSubmit={studentManualLogin}>
                <div className="fgrp">
                  ...
                  <div style={{ display: "flex", alignItems: "center", background: "var(--bg)", border: "1px solid var(--bdr)", borderRadius: "8px", overflow: "hidden" }}>
                    <span style={{ padding: "12px", background: "var(--bg2)", color: "var(--t2)", fontWeight: 700, borderRight: "1px solid var(--bdr)", fontSize: ".9rem" }}>VIT-26-</span>
                    <input type="text" value={manualId} onChange={(e) => setManualId(e.target.value.replace(/\D/g, '').slice(0, 5))} placeholder="66653" style={{ width: "100%", padding: "12px", border: "none", background: "transparent", color: "var(--t1)", outline: "none", fontWeight: 600 }} />
                  </div>
                </div>
                <div className="fgrp">
